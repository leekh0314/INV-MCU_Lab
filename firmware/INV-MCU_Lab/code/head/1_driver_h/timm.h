...
#define _00_TMM_PWMOP_HAZARD_DIASBLE                 (0x00U) /* hazard measure disabled / 危险防护禁止 */
#define _40_TMM_PWMOP_HAZARD_ENASBLE                 (0x40U) /* hazard measure enabled / 启用危险防护 */
/* Output forced cutoff source edge/ release edge selection (IN_EG) */
#define _00_TMM_PWMOP_FALLING_RELEASE                (0x00U) /* rising edge: Output forced cutoff/falling edge: Output forced cutoff release / 上升沿触发强制截止，下降沿解除 */
#define _20_TMM_PWMOP_RISING_RELEASE                 (0x20U) /* rising edge: output forced cutoff release/falling edge: output forced cutoff / 上升沿解除强制截止，下降沿触发 */
/* Cutoff source selection (IN_SEL1,IN_SEL0) */
#define _00_TMM_PWMOP_NO_CUTOFF                      (0x00U) /* NO cutoff source selection / 不选择任何截止源 */
#define _08_TMM_PWMOP_COMP_CUTOFF                    (0x08U) /* comparator 0 output / 比较器0输出 */
#define _10_TMM_PWMOP_INTP_CUTOFF                    (0x10U) /* INTP0 pin input / INTP0 引脚输入 */
#define _18_TMM_PWMOP_ELC_CUTOFF                     (0x18U) /* event input from ELC / 来自 ELC 的事件输入 */
/* When software release is selected: Software release timing selection (ACT) */
#define _00_TMM_PWMOP_SOFT_IMMEDIAT_RELEASE          (0x00U) /* immediately release / 立即解除 */
#define _04_TMM_PWMOP_SOFT_CONDITION_RELEASE         (0x04U) /* conditional release / 条件解除 */
/* When software release is selected: Output cutoff release control (HZ_REL) */
#define _00_TMM_PWMOP_SOFT_RELEASE_REMAIN            (0x00U) /* output forced cutoff remains / 维持强制截止状态 */
#define _02_TMM_PWMOP_SOFT_RELEASE_RESUM             (0x02U) /* output forced cutoff is released and pulse output is resumed / 解除并恢复输出 */
/* Output forced cutoff release mode selection (HS_SEL) */
#define _00_TMM_PWMOP_HARDWARE_RELEAS                (0x00U) /* released via hardware / 通过硬件解除 */
#define _01_TMM_PWMOP_SOFTWARE_RELEAS                (0x01U) /* release via software / 通过软件解除 */
